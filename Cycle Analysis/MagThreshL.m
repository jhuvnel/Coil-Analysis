...
%         cutOff = 100;
    ...
    if isempty(Results)
        handles.s.pullIndsL(i) = s+imag-1;
        handles.s.Misalign3DL(i,:) = [LE_LARP(s+imag-1) LE_RALP(s+imag-1) LE_Z(s+imag-1)];
        ...
        handles.s.MisalignL(i) = acosd(cosT);
        handles.s.maxMagL(i) = mag;
    else
